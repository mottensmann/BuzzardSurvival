---
title: "R-code for 'Surviving in a changing world: weather and juvenile condition matter for a long-lived avian predator, but blood parasites do not appear to'"
...
knitr::opts_chunk$set(
  ...
  strip.white = TRUE,collapse = TRUE, fig.align = "center", cache = F,
  warning = F, message = F)
```
...
do_analysis.barker = function(dp.barker, ddl.barker) {
  ...
  ## Null model: Only age-specific survival rates
  ## ---------------------------------------------------------------------------
  ...
  ## Adding Hatching date to account for seasonal effects on juvenile survival 
  ## ---------------------------------------------------------------------------
...
## output data files:
## -----------------------------------------------------------------------------
models.barker.adj_1 <- RMark::remove.mark(models.barker.adj, model.numbers = 249:496)
models.barker.adj_2 <- RMark::remove.mark(models.barker.adj, model.numbers = 1:248)
...
## Clean-up debris (=unused temporary files)
## -----------------------------------------------------------------------------
cleanup(ask = FALSE)
...
models <- data.frame(model.number = 1:(length(models.barker.adj) - 1),
                     model = sapply(models.barker.adj[1:496], function(x) x[["model.name"]]))
...
* `r supported.models.barker[[5]]$model.name`
* `r supported.models.barker[[10]]$model.name`
...
Explore survival rates in relation to fixed effects.
...
## Assemble multi-panel figure, Figure 2 in the paper
## -----------------------------------------------------------------------------
...
```{r, echo=T, eval=F}
ggsave(plot = Fig_2_new, filename =  "plots/Fig_2.pdf", 
        device = "pdf", dpi = 1200, width = 6, height = 6, units = "in")
ggsave(plot = Fig_2_new, filename = "plots/Fig_2.png", 
        device = "png", dpi = 1200, width = 6, height = 6, units = "in")
```

##### Resighting rates (R)

Explore re-sighting rates:
...
p.res <- 
  ...
  theme(
    ...
    annotate("text", x = 2007 , y = Inf, hjust = 0, vjust = 1.5,
             label = paste0("adj. R²: ", round(lm.mod$adj.r.squared,3), 
                            "\np < 0.001"))
```
...
## to have survived at least until reaching sub-adulthood
resighting_data <- as.data.frame(read_csv("data/resighting_data.csv")) %>% 
  filter(Ring %in% ringing_data$Ring)
...
## -----------------------------------------------------------------------------
pred.leuco <- 
  plot_model(model.1, type = "pred", terms = "Age [all]",
             title = "", ci.lvl = 0.95, show.data = T, 
             line.size = 1.22, colors = 'gs') +
  theme_article() +
  theme(axis.ticks.y = element_blank(),
        axis.title.y = element_blank(),
        axis.text.y = element_blank(),
...
```{r, eval=FALSE}
## Split capture history (ch) into individual occasion
## -----------------------------------------------------------------------------
...
## separate L(ive) and D(ead) occasions.
...
## D: '2' denotes live resightings and '1' dead recovery
## -----------------------------------------------------------------------------
...
## D: 2 denotes live resighting and 1 dead recovery
## -----------------------------------------------------------------------------
output <- matrix(
  data = 0,
...
To summarise heterogeneity in resighting  histories creates image for each cohort
...
There is strong decline in the number of re-sightings per age class
...
Summarise climate variables and association to large scale variation in NAO
...
```{r}
## subset dataset to include all-wingtaged individuals as used above
## -----------------------------------------------------------------------------
ringing_data <- filter(
  ringing_data,
  # Subset to wing-tagged only
...
## Code Brood Rank as a factor with four levels
## -----------------------------------------------------------------------------
ringing_data$Rank[ringing_data$Rank %in% c("4", "5")] <- "4/5"

## LMER testing for differences in average condition based on brood rank 
## -----------------------------------------------------------------------------
mod <- lmer(BCI ~ Rank + Brood_Size + (1|Brood_ID) + (1|Year), data =  ringing_data)
tab_model(mod, show.stat = T, show.se = T, show.ci = F, string.est = 'est', string.se = 'se', string.stat = 't')
...
## create data frame to indicate significance of contrasts
## -----------------------------------------------------------------------------
...
Fig_S7 <- ggplot(pred, aes(x = Rank, y = predicted, fill = Rank, col = Rank)) +
  ...
  geom_errorbar(aes(ymin = predicted - std.error,
                ...
                size = 8, width = 0) +
  scale_y_continuous(limits = c(-1.2, 0.4), breaks = seq(-1,0.4,0.2)) +
  geom_point(col = "Black") +
  theme_article() +
  theme(legend.position = "none") +
  stat_n_text(data = ringing_data, aes(x = Rank, y = BCI),
              y.pos = -1.2, vjust = 0, y.expand.factor = 0.5) +
